...
var FEED_AUTHOR = &FeedAuthor{Name: "Yury Egorenkov", Email: "yury.egorenkov@gmail.com"}

var SITE_BASE = func() string {
	if FLAGS.DEV {
		return "http://localhost:" + SERVER_PORT
	}
	return "https://yury.co"
}()
	...
	*/
	case bf.Heading:
		...
		if entering {
			...
			if node.HeadingID != "" {
				out.Write([]byte(`<span style="margin-left: 0.5rem;"><a href="#` + node.HeadingID + `" class="heading-anchor" aria-hidden="true"></a></span>`))
			}
